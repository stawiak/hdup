...
class MQLExecutorTest extends FlatSpec with ShouldMatchers {
	...
	"MQL executor" should "generate single request and a period" in {
		...
		println(requests)
		requests should be (List(MeasurementReadRequest("msmt","a","a","a", new Interval(1, 2))))
	}
	...
	it should "generate single rollup request and a period" in {
		...
		val requests = new MQLExecutor(parser.parseAll(parser.mql, mql).get).generateExecutePlan
		println(requests)
		requests should be (List(RollupReadRequest("a","a",new Interval(1, 2))))
	}
	...
	it should "generate 2 requests for a union" in {
		...
		val requests = new MQLExecutor(parser.parseAll(parser.mql, mql).get).generateExecutePlan
		println(requests)
		requests.head should be (MeasurementReadRequest("msmt","a","a","a", new Interval(1, 2)))
		requests.tail should be (List(RollupReadRequest("a","a",new Interval(2, 3))))
	}

...
class RestService extends Actor with SprayActorLogging {
	...
	def receive = {
		...
		case HttpRequest(GET, "/msmt", _, _, _) =>
			...
			sender ! HttpResponse(entity = HttpBody(contentType = ContentType.`application/json`, string = tsd.toJSONString))
